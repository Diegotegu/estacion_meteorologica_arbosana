...
while (Serial1.available()) {
    Serial.print(buff);
    buff = Serial1.read(); 
    if (buff==13 && Serial1.read()==10){
             end_string=true;
             Serial.println("final de la cadena");
    }
    respuesta += buff;
    delay(100);
    if (millis()-t_start >30000) {
      Serial.println("limite alcanzado");
      end_string_time=true;
      }    
  }
  //end_string=false;
  Serial.println();
  ...
  direccion = secciones[1].toFloat();
  velocidad = secciones[4].toFloat();
  temperatura = secciones[6].toFloat();
  humedad = secciones[8].toFloat();
  presion = secciones[9].toFloat();
  lluvia = secciones[10].toFloat(); //lluvia acumulada e intensidad de lluvia
  lluvia_intensidad = secciones[12].toFloat();
  granizo = secciones[13].toFloat();//granizo acumulado e intensidad de granizo
  granizo_intensidad = secciones[15].toFloat();


  msg.direccion = convertoFloatToUInt16(direccion, 360);
  msg.velocidad = convertoFloatToUInt16(velocidad, 50);
  msg.temperatura = convertoFloatToUInt16(temperatura, 60, -60);
  msg.humedad = convertoFloatToUInt16(humedad, 0, 100);
